...
- **`data_new/`(2026-08-18~)** — 김태윤이 처음부터 다시 짠 데이터 파이프라인.
  ...
  **팀 결정으로 앞으로는 `data_new/` 기준으로만 작업한다** — `data/`(구)를 실제로
  ...
  `data_new/README.md`(파이프라인·매칭 설계·수치·한계 전부 이 파일 하나에 정리됨).
...
10. **매칭 신뢰도 임계값 0.8 확정 — 같은 날 12번에서 재론·완전 교체됨.**
    `FUZZY_NAME_THRESHOLD` 자체가 코드에서 사라졌다(exact match 방식으로
    전환) — 지금은 없는 메커니즘이라 항목만 번호 유지, 내용은 12번 참고.
...
12. **(10번 재론·교체) 매칭을 로마자 유사도(0.8 임계값)에서 한글
    ...
    끝부분이 겹쳐서 점수가 높게 나옴)을 없앨 수 있었다(전체 설계·근거는
    `data_new/README.md`). matched
    ...
    쓰기로 정리(1,262→1,234척) — 어선원부 관련 수집·정규화·1·2단계
    매칭 스크립트 전부 삭제. 같은 날 마지막으로, 사람 스팟체크에서
    verified의 63.8%가 같은 TAC 배를 여러 GFW 선박이 동시에 주장하는
    중복배정이었던 게 발견돼(예: TAC "한성호" 1척이 GFW 9척에 매칭)
    TAC 쪽 유일성을 강제하는 5단계를 추가함(1,234→713척, 13.4%). 이어서
    verified 25쌍 스팟체크로 "NO.6 KYUNG YANG"이 IMO번호·601GT 원양
    참치연승선인데 TAC의 8.55톤 근해소형선과 오매칭된 걸 발견 — GFW
    `registryInfo` 있는 선박(21척, 0.4%)을 후보풀에서 통째로 제외함
    (713→712척) — **지금 이 수치(712척, 13.4%)가 최종.** 전체 설계·
    발견 과정·알려진 한계는 `data_new/README.md` 하나에 정리돼 있다.
