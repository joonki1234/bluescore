...
# .env를 환경변수로 올린다. uvicorn은 app.py(Streamlit)와 다른 프로세스라
# app.py의 load_dotenv가 여기까지 오지 않는다 — 이게 없으면 질의응답이
# 조용히 llm_disabled 폴백으로 떨어진다. python-dotenv가 없어도 앱은 돌아간다.
try:
...
def create_app(
    ...
    def review_score_run(score_run_id: str, request: ReviewDecision) -> ReviewDetail:
        """여신 심사 결정을 저장한다. 이의제기가 접수돼 있으면 함께 매달리고,
        없어도 결정을 남길 수 있다 — 심사는 이의제기와 무관한 절차다.
        """
