...
durationHours 등)는 GFW 원본 응답 구조와 다르다 — 원본은 vessel.id,
position.lat/lon처럼 타입별 하위객체로 중첩돼 있다 — 이 스크립트가
그 변환을 전담한다.
...
def _duration_hours(start: str, end: str) -> float:
    """GFW `fishing.averageDurationHours`는 100% null이다(중복 제거 후
    276,562건 확인). 대신 매 이벤트에 항상 있는 start/end(결측 0건)로
    직접 계산한다."""
...
def normalize_event(raw: dict) -> dict:
    ...
    필드 매핑: id -> eventId, vessel.id -> vesselId,
    position.lat/lon -> latitude/longitude, durationHours는 GFW 필드를
    그대로 안 쓰고 start/end로 직접 계산(_duration_hours 참고).
    """
