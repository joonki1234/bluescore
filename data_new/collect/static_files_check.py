...
이 셋은 API가 막혀있어(TAC는 공공기관 전용) data.go.kr에서 수동으로 내려받은
파일을 그대로 raw/에 둔 것이라 별도 수집 스크립트의 검증게이트를 거치지
못했다. 이 스크립트가 그 구멍을 메운다: 컬럼 수가 기록해둔 값과 같은지
확인하고, 우리가 언제 이 파일을 확인했는지를 메타데이터로 남긴다.
