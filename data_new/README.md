...
잡고, TAC 할당 승인정보와 한글 이름으로 직접 대조해 개별 선박을
...
└── processed/      정규화·매칭 결과와 API 캐시(5개 git 추적 —
                      서비스 직접 입력 3개 + 재현·캐시용 2개)
```
...
events_with_weather.jsonl.gz`(기상결합 이벤트 275,782건). `score/
real_vessel_input.py`가 앞의 선박 파일 2개를 공용 선박 입력으로
변환하고, `services/real_scoring.py`가 이벤트 파일을 읽어
`score/axis_a_pressure.py`·`score/real_axis_b_input.py`에 연결한다.
`matchTier != "verified"`인 행은 `tac`/`mof`가 비어 있어 톤수가
안 잡히므로 B축에서 제외되지만, 조업 이벤트가 있으면 A축은 계산된다.
...
금성호로 오매칭). 대신 GFW 영문명 4,662척을 검토해 한글 후보를
미리 부여해둔 CSV(`reference/gfw_korean_name_candidates.csv`,
3,878척 부여, 83%)와 한글 원문끼리
...
| 구분 | 척수 | 비중 | 비고 |
|---|---:|---:|---|
| **verified — 규칙 통과** | **712** | **13.4%** | 한글명 일치 + 양쪽 숫자 충돌 없음 + 위치 확인(≤150km) + TAC 유일성 |
| held_multi — TAC 매칭 보류 | 1,769 | 33.2% | 동명이선, TAC 중복배정 패배분, 거리 미확인 |
| no_korean — TAC 매칭 불가 | 777 | 14.6% | GFW 한글 후보 자체가 없음(범용 영문명 등) |
| unmatched — TAC 매칭 불가 | 2,065 | 38.8% | TAC에 같은 이름 없음, GFW 이름 없음, 원양선 추정 |

held_multi·unmatched·no_korean 선박도 조업 이벤트가 있으면 A축은
계산된다 — 근거 없이 톤수를 붙이지 않을 뿐이라, 톤수가 필요한 B축만
제외된다.

## 알려진 한계

- **정밀도는 아직 체계적으로 실측하지 못했다**: 구조적 오탐 경로를
  없애고 사람 스팟체크로 실제 오류 2건을 찾아 고쳤지만, verified
  712척 전체를 독립 정답셋과 대조한 precision은 아니다. 선박번호와
  MMSI를 함께 가진 한국 AIS 등록 데이터셋을 확보하면 검증할 수 있다.

- **150km 임계값**: 법령을 확인하고 민감도 분석(50km→538척,
  300km→1,723척)까지 해봤지만 근거가 될 자연스러운 경계는 없어,
  정밀도 우선 원칙 아래 보수적으로 잡은 정책값으로 두고
  `LOC_VERIFIED_KM` 상수 하나로 재조정 가능하게 설계했다. 정확한
  값은 현재 데이터셋만으로는 검증할 수 없어 한계로 남긴다 — 같은
  한국 AIS 등록 데이터셋을 확보하면 경험적으로 재조정 가능하다.

- **기상 결합은 거리·시간 상한이 없다**: 이벤트마다 가장 가까운
  관측소·시각을 무조건 붙인다(`attach_weather.py`). 실측 기준
  관측소 거리는 중앙값 24.3km·95% 101.0km·최대 342.9km(150km
  초과 2,791건), 관측 시차는 중앙값 2.5분·최대 768.4분(60분 초과
  216건) — 일부는 시공간적으로 멀리 떨어진 관측값이 붙는다.
