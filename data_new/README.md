...
담당: 김태윤. `bluescore/data/`(구 파이프라인)는 참고하지 않고 처음부터
다시 설계했다. 이 문서 하나로 데이터 파이프라인 전체 — 무엇을 왜
이렇게 만들었는지, 최종 수치가 뭔지, 한계가 뭔지 — 를 설명한다.
...
**왜 이렇게 됐나**: 처음엔 GFW 자기신고 영문명을 로마자로 변환해 TAC
한글명과 문자열 유사도(fuzzy)로 비교했다. 사람 라벨링 80쌍 검증 결과
정밀도 약 75%(matched 2,881척/54.1%, 추정 오탐 ~720척)였는데, 로마자
유사도는 서로 다른 실제 이름인데 로마자로 바꾸면 끝부분이 겹쳐서
점수가 높게 나오는 구조적 오탐이 있었다(예: EUNSEONGHO(은성호)가
금성호로 오매칭). GFW 영문명 4,662척 전체를 사람이 직접 한글로
재변환한 데이터(`reference/gfw_korean_name_candidates.csv`, 규칙기반
반자동 프로세스 — letterPart만 변환, 숫자·"호" 접미사 제외, 애매하면
후보 최대 3개를 `|`로 구분, 해수부 선박마스터 자료로 보강하되
음역결과와 일치할 때만 채택, 확신 없으면 보수적으로 빈칸)가 생기면서
로마자 변환 자체를 건너뛰고 한글 원문끼리 직접 비교하는 방식으로
전면 교체했다. 오탐 하나가 B축 점수를 그 배 단위로 완전히 틀어지게
만들 수 있어 커버리지보다 정밀도를 우선하기로 결정했다.
...
5. **TAC 쪽 유일성 강제** — 1~4단계는 GFW 선박마다 독립 판정이라,
   ...
   TAC의 같은 배 하나를 동시에 주장할 수 있다(사람 스팟체크로 발견 —
   도입 전 verified의 63.8%가 이 충돌에 걸려 있었음, TAC "한성호" 1척이
   GFW 9척에 매칭된 극단 사례 확인). 4단계까지 마친 뒤 TAC 등록번호별로
   재집계해서, 같은 TAC 배를 여러 GFW 선박이 주장하면 조업위치
   최근접 하나만 verified로 남기고 나머지는 held_multi로 되돌린다.

**후보풀에서 제외한 것**: 어선원부(2006년 처리배치 일부, 전부
비현행이라 신뢰도 낮음, 기여분 0.1%뿐), MOF(이름검색이 어선보다
상선 위주로 편향), GFW `registryInfo`가 있는 선박(21척, 대부분 원양
대형선단 — 스팟체크로 IMO번호·601GT 원양 참치연승선이 TAC 8.55톤
소형선과 오매칭된 확실한 사례 발견해서 제외).
...
어떻게 여기까지 줄었는지: matched 2,881척(로마자 유사도) → verified
1,262척(한글 직접비교 도입) → 1,234척(어선원부 제외) → 713척(TAC
유일성 강제) → **712척(원양선 제외, 최종)**.
...
- **카카오 지오코딩 좌표 자체는 스팟체크 안 함**: 위치확인율(TAC
  96.4%)은 "찾았다/못 찾았다"만 검증했지, 찾은 좌표가 진짜 그 항구인지
  (동명 지명 오매칭 가능성)는 검증하지 않았다.
