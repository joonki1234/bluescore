...
# z-score 정규화 후 결합하는 방식으로 바꿔서, 같은 vesselId라도
# axis_a_pressure_raw·shap_factors 값이 달라진다. 버전을 안 올리면
# services/workflow.py의 캐시 신선도 체크가 이 변경을 못 알아채고 옛
# 결과를 계속 돌려준다(REAL_DATA_SNAPSHOT_ID 갱신 때 이미 한 번 겪은
# 문제와 같은 종류).
