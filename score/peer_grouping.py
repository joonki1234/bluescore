...
주의 (미확정 파라미터, CLAUDE.md "미확정 항목" 참고):
    - 톤수대 폭(TONNAGE_BAND_WIDTH)
    - 유사군 최소 표본 기준(MIN_PEER_GROUP_SAMPLE_SIZE)
    둘 다 팀 확정 전 잠정값이다.
...
# 유사군 최소 표본 기준(척) — 잠정값. 이보다 적으면 백분위 정규화 대신
# insufficientSample로 판정한다.
MIN_PEER_GROUP_SAMPLE_SIZE = 20
