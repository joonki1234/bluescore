...
주의 (미확정 파라미터, CLAUDE.md "미확정 항목" 참고):
    ...
    - 유사군 최소 표본 기준(MIN_PEER_GROUP_SAMPLE_SIZE) = 10척(CLAUDE.md
      확정된 규칙 참고). data_new/ 실측(1,079개 그룹, 중앙값 2척) 기준
      20척은 커버리지 42.6%, 10척은 61.0%로, "통계적으로 최소 방어 가능한
      하한(10척=10% 해상도)"과 "실산출 커버리지" 사이의 트레이드오프다.
      근본 원인(그룹이 과도하게 잘게 쪼개지는 것)은 gearType 처리 방식과
      얽혀 있어, 이후 개선 여지가 크다.
