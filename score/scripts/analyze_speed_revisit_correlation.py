...
`AXIS_A_COST_PER_KNOT`(services/scoring.py의 트레이드오프 계수 — "속도를
낮추면 해상 체류가 길어져 자원 압력이 깎인다")가 실제로 근거가 있는지,
data_new/ 실측 데이터로 "평균 속도"와 "평균 재방문 간격" 사이에 상관관계가
있는지 확인한다.

배경: axis_a_pressure.py에는 속도가 재방문압력에 미치는 영향을 계산하는
공식이 아예 없다 — 그래서 이 계수는 원래부터 근거 없는 추측값이었고,
B축 계수를 실제 물리식으로 바꾸면서 그 부작용(시뮬레이터 반작용 부호 역전)이
드러났다. 상관관계가 실제로 있으면 방향과 크기를 실측으로 추정하고, 없으면
계수를 0에 가깝게 두는 게 근거 없는 값을 그대로 두는 것보다 낫다는 판단의
근거가 된다.
